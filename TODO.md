...
### 2026-03-18 (Phase 3 完成)
- **完成电脑操控增强模块**：
  - 新增 close_app 命令（关闭应用进程）
  - 新增 list_running_processes 命令（列出运行中进程）
  - 新增 ProcessInfo 模型
  - AppLauncher 组件（启动/关闭应用）
  - BrowserControl 组件（打开网址、收藏夹）
- **优化文件管理器用户体验**：
  - 文件操作成功后显示 Toast 提示
  - 重命名、删除、创建文件夹反馈
...
- **完成形象声音模块**：
  - 新增 Avatar 组件（支持不同尺寸和动画效果）
  - 新增 AvatarSettings 设置页面
  - 20 个预设形象（机器人、猫咪、狐狸、企鹅、小熊等）
  - 支持自定义头像 URL
  - 侧边栏标题显示当前选择的头像
  - 对话消息中显示 AI 头像
  - AI 回复时头像说话动画效果
  - 声音选择器和语音参数调节（已在之前完成）
- **修复 Embeddings API 兼容性问题**：
  - 后端增加详细错误提示
  - 前端添加供应商兼容性检查

### 2026-03-18 (桌宠模式)
- **完成桌宠模式基础功能**：
  - 新增 DesktopPet 组件（可拖动、点击菜单、动画效果）
  - 新增 PetWindow 页面（透明窗口）
  - 新增 PetSettings 设置组件（开关、显示/隐藏）
...
**当前优先级**: Phase 5 桌宠模式基础完成，待测试

下一步：
1. **测试桌宠功能**：运行 `pnpm tauri dev` 测试透明窗口和交互
2. **Phase 3 完善**：网页截图功能
3. **Phase 6: 微信互联模块** - 微信窗口检测、消息发送/读取
4. **Phase 5 增强**：Live2D 支持（可选）
...
| `feat(control): 完成 Phase 3 电脑操控增强模块` | 应用管理、浏览器控制 |
| `feat(ui): 优化文件管理器用户体验` | 操作成功 Toast 提示 |
| `fix(knowledge): 修复 Embeddings API 兼容性问题` | 供应商兼容性检查 |
| `feat(avatar): 添加形象设置模块` | 预设形象、自定义头像 |
